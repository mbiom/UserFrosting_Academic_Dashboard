<div id="p<?php echo context::global_filter($_ctx->posts->post_id,0,0,0,0,0,0,'EntryID'); ?>" class="post short <?php if (($_ctx->posts->index()+1)%2 == 1) { echo 'odd'; } ?> <?php if ($_ctx->posts->index() == 0) { echo 'first'; } ?>" lang="<?php if ($_ctx->posts->post_lang) { echo context::global_filter($_ctx->posts->post_lang,0,0,0,0,0,0,'EntryLang'); } else {echo context::global_filter($core->blog->settings->system->lang,0,0,0,0,0,0,'EntryLang'); } ?>" role="article">
	...
	<?php if($_ctx->posts->isExtended()) : ?>
		<div class="post-content">
			...
			<?php echo context::global_filter($_ctx->posts->getExcerpt(0),0,0,0,0,0,0,'EntryExcerpt'); ?>
		</div>
		<p class="post-read-it"><a href="<?php echo context::global_filter($_ctx->posts->getURL(),0,0,0,0,0,0,'EntryURL'); ?>"
	...
	<?php if(!$_ctx->posts->isExtended()) : ?>
		<div class="post-content">
			<?php echo context::EntryFirstImageHelper('t',0,'post-icon',0,0,0); ?>
			<p>
				<?php echo context::global_filter($_ctx->posts->getContent(0),1,1,300,0,0,0,'EntryContent'); ?>
				<?php if (strlen(context::global_filter($_ctx->posts->getContent(0),1,1,0,0,0,0,'EntryIfContentCut')) > strlen(context::global_filter($_ctx->posts->getContent(0),1,1,300,0,0,0,'EntryIfContentCut'))) : ?>
